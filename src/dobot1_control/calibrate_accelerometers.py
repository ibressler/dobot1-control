...
from dobot1_control import DobotDriver, DobotKinematics
from dobot1_control.DobotBase import FRONT, REAR, arrayToStr
...
def display_accelerometer_data(driver, kinematics, ret):
    if ret[0]:
        if driver.isFpga():
            ...
            print(
                f"Angles rear: {rear_deg:10f}°, front: {front_deg:10f}° | "
...
def positions_mode(driver, kinematics, positions):
    ...
    for i, pos in enumerate(positions):
        ...
        input(f"-> Move the arm to cartesian position {i} {pos}, "
              f"angles {arrayToStr(np.rad2deg(angles[-1]))}, and press <enter>...")
        sensors.append(driver.GetAccelerometers())
        display_accelerometer_data(driver, kinematics, sensors[-1])

    offset_rear = float(
        (sensors[0][1] * np.cos(angles[1][REAR]) - sensors[1][1] * np.cos(angles[0][REAR]))
        / (np.cos(angles[1][REAR]) - np.cos(angles[0][REAR]))
    )
    offset_front = float(
        (sensors[0][4] * np.sin(angles[1][FRONT]) - sensors[1][4] * np.sin(angles[0][FRONT]))
        / (np.sin(angles[1][FRONT]) - np.sin(angles[0][FRONT]))
    )
    accelConv0 = float((sensors[0][1] - offset_rear) / np.sin(np.pi * 0.5 - angles[0][REAR]))
    ...
    print(
        f"   accelOffset=({offset_rear:.2f}, {offset_front:.2f}), "
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        nargs=3,
        default=positionDefaults["pos1"],
        metavar=("X", "Y", "Z"),
        help=f'First position for calibration, default: {positionDefaults["pos1"]}',
    )
    parser.add_argument(
        "--pos2",
        type=float,
        nargs=3,
        default=positionDefaults["pos2"],
        metavar=("X", "Y", "Z"),
        help=f'Second position for calibration, default: {positionDefaults["pos2"]}',
    )
    parser.add_argument(
        "--offset",
        type=float,
        nargs=2,
        default=positionDefaults["offset"],
        metavar=("H", "V"),
        help="End effector offset: (horizontal, vertical) distance "
